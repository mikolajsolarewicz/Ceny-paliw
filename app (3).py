...
def _fetch_fuel() -> pd.DataFrame:
    ...
    frames = []
    for fuel in FUELS:
        r = requests.get(f"https://api.paliwo.today/api/prices?type={fuel}", timeout=30)
        r.raise_for_status()
        ...
        frames.append(rec[["date", fuel if fuel in rec else "price"]]
                      .rename(columns={"price": fuel}).set_index("date"))
...
def _fetch_usd() -> pd.DataFrame:
    ...
    while start < today:
        ...
        r = requests.get(f"https://api.nbp.pl/api/exchangerates/rates/a/usd/"
                         f"{start}/{end}/?format=json", timeout=30)
...
def _fetch_brent() -> pd.DataFrame:
    """Notowania Brent (BZ=F) z Yahoo Finance przez yfinance."""
...
def fetch_live() -> pd.DataFrame:
    fuel, usd, brent = _fetch_fuel(), _fetch_usd(), _fetch_brent()
...
def get_data() -> tuple[pd.DataFrame, str, str]:
    """Najpierw probuje pobrac PRAWDZIWE dane; w razie problemu - dane przykladowe."""
    try:
        df = fetch_live()
        if len(df) > 100:
            info = f"pobrano {pd.Timestamp.now():%Y-%m-%d %H:%M}, zakres do {df['date'].max():%Y-%m-%d}"
            return df, "live", info
        raise ValueError("zbyt malo danych z API")
    except Exception as exc:  # noqa: BLE001
        return generate_sample(), "sample", f"{type(exc).__name__}: {exc}"[:300]
...
with st.sidebar:
    ...
    if source == "live":
        ...
        st.warning("Automatyczne pobranie nie powiodlo sie - pokazuje dane PRZYKLADOWE. "
                   "Kliknij ponizej, aby sprobowac jeszcze raz.")
        with st.expander("Szczegoly bledu"):
            st.code(info)
    if st.button("\U0001F504 Pobierz dane na nowo"):
        st.cache_data.clear()
        st.rerun()
