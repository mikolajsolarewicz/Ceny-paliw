...
@st.cache_data(show_spinner="Przygotowuje dane...")
def get_data() -> tuple[pd.DataFrame, str]:
    """Probuje wczytac prawdziwe CSV; w razie problemu generuje dane przykladowe."""
    try:
        df = load_from_csv()
        meta = DATA_DIR / "_meta.json"
        src = json.loads(meta.read_text()).get("source", "live") if meta.exists() else "live"
        if len(df) > 100:
            return df, src
    except Exception:
        pass
    return generate_sample(), "sample"
...
with st.sidebar:
    ...
    if source == "live":
        ...
        st.warning("Dane PRZYKLADOWE (syntetyczne). Aby uzyc prawdziwych, "
                   "uruchom lokalnie fetch_data.py i wgraj katalog data/.")
