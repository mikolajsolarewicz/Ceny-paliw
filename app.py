...
def get_data() -> tuple[pd.DataFrame, str, str]:
    ...
    if fuel is not None and usd is not None and brent is not None:
        try:
            ...
            df["brent_pln_l"] = df["brent_usd"] * df["usdpln"] / LITERS_PER_BARREL
            if len(df) >= 25:
                return df, "live", f"{df['date'].min():%d.%m.%Y}\u2013{df['date'].max():%d.%m.%Y}, {len(df)} dni"
...
SPAN_DAYS = (df_all["date"].max() - df_all["date"].min()).days
MAXLAG = int(min(30, max(7, SPAN_DAYS // 3)))

st.markdown("<p class='big-title'>Ceny paliw w Polsce</p>", unsafe_allow_html=True)
st.markdown(f"<p class='subtitle'>Trendy i reakcja cen na rynek ropy "
...
with t1:
    ...
    pb = df_all["PB95"]
    lo_d = df_all.loc[pb.idxmin(), "date"]
    hi_d = df_all.loc[pb.idxmax(), "date"]
    st.markdown(f"<div class='insight'>W analizowanym okresie benzyna 95 wahala sie od "
                ...
                f"({hi_d:%m.%Y}).</div>", unsafe_allow_html=True)
...
with t3:
    ...
    if SPAN_DAYS < 700:
        st.info(f"Analiza sezonowosci wymaga danych z co najmniej okolo 2 lat. "
                f"Dostepny zakres realnych danych ({SPAN_DAYS} dni) jest na to za krotki, "
                f"by wiarygodnie wyznaczyc roczny wzorzec - dlatego ta sekcja pozostaje "
                f"nieaktywna dla biezacych danych na zywo.")
    else:
        tmp = df_all.copy()
